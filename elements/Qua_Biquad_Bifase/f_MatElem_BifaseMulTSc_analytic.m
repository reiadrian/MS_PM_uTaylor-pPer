function [kt,fint,u,eps_new,phi_new,p_new,eps_fluct,phi_fluct,p_fluct,...
            ...
            f_MatElem_BifaseMulTSc_analytic(u,u_old,yy,hvar_old,aux_var,...
...
%Micro-poro presiones fluctuantes o delta de macro-poro presiones al paso "n"
p_fluct_n = zeros(4,1);
%############################################################################################
...
if conshyp == 15 % caso conshyp=15 RVE de caso MULTIESCALA    
    %Inicializacion
...
%############################################################################################
    ...
    switch conshyp      
        %###############################################################################
        case 15 %Modelo bifasico con solido elastico (saturado) con metodo multiplicadores de Lagrange
            %Caso Multi-escala
           p_new_sigma=p_new;
    ...
    if conshyp==15  %Modelo bifasico con solido elastico (saturado) con metodo de
