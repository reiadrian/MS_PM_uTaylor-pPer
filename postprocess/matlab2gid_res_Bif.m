function matlab2gid_res_Bif(iStep,in,u,c_GdlCond,e_DatSet,e_VarEst,e_VarAux,DefMacro,GradPorMacro,udTotal,upTotal,Fext,Fint,ELOC,e_VG)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%El primer elemento logico es para analisis MULTIESCALA y el segundo elemento logico es para analisis RVE
if e_VG.esME || isfield(e_VG,'omegaMicro') 
           ...
           case 0
            ...
            case 1 %AA
               if ndime==2
                   ...
                   %#%#%#%#%#%#%#%#%#%#%#%#%#%#%#%#%#%#%#%#%#%#%#%#%#%#%#%#%#%#%#%#%#%#%#%#%#%#%#%#%#%#%#%#%#%#%#%#%
                   %Reacciones a desplazamientos
                   reac = Fint-Fext;
                   fprintf(fid_res,['Result "Reacciones_Desp',tipoDesp,'" "Load Analysis" %d Vector OnNodes\n'],iStep);
                   ndn_d=e_VG.ndn_d;
                   reac_u=reac(e_VG.pos_dG);
                   fprintf(fid_res,'ComponentNames "FX" "FY"\n');
                   fprintf(fid_res,'Values\n');
                   format = ['%d',repmat(' %.15g',1,ndn_d),'\n'];
                   fprintf(fid_res,format,[in';reshape(reac_u,ndn_d,[])]);   %[in,u(1:ndime:end),u(2:ndime:end)]'
                   fprintf(fid_res,'End Values\n');
                   %#%#%#%#%#%#%#%#%#%#%#%#%#%#%#%#%#%#%#%#%#%#%#%#%#%#%#%#%#%#%#%#%#%#%#%#%#%#%#%#%#%#%#%#%#%#%#%#%
...
%                    pos_pG=3*in_esq;
                   %############################################################################################
                   if e_VG.conshyp==14 || e_VG.conshyp==15 || e_VG.conshyp==16  || e_VG.conshyp==17 || e_VG.conshyp==50 || e_VG.conshyp==60 ...
                            || e_VG.conshyp==61
                       pos_pG=3*in;
                   elseif e_VG.conshyp==0
                       pos_pG=4*in-1;
                   end
                   %############################################################################################
                   up=u(pos_pG);
%                    inp=in(in_esq);
                   ...
                   %#%#%#%#%#%#%#%#%#%#%#%#%#%#%#%#%#%#%#%#%#%#%#%#%#%#%#%#%#%#%#%#%#%#%#%#%#%#%#%#%#%#%#%#%#%#%#%#%
                   %Reacciones a poro presiones
                   %############################################################################################
                    reac = f_porp_nint(reac,e_DatSet,e_VG); %AA: obtiene los valores de reacciones de poropresiones en los nodos internos
                    %############################################################################################
                   fprintf(fid_res,['Result "Flujo',tipoDesp,'" "Load Analysis" %d Scalar OnNodes\n'],iStep); %Scalar
                   ndn_p=e_VG.ndn_p;
%                    in_esq=e_VG.in_esq;
%                    pos_pG=3*in_esq;
                   %############################################################################################
                   ...
                   reac_p=reac(pos_pG);
%                    inp=in(in_esq);
                   fprintf(fid_res,'ComponentNames "Flujo"\n');
                   fprintf(fid_res,'Values\n');
                   format = ['%d',repmat(' %.15g',1,ndn_p),'\n'];
                   fprintf(fid_res,format,[in';reshape(reac_p,ndn_p,[])]);   %[in,u(1:ndime:end),u(2:ndime:end)]' %inp
                   fprintf(fid_res,'End Values\n');
...
%                    inp=in(in_esq);
                   ...
                   fprintf(fid_res,'Values\n');
                   format = ['%d',repmat(' %.15g',1,ndn_p),'\n'];
                   fprintf(fid_res,format,[in';reshape(upTotal,ndn_p,[])]);   %[in,u(1:ndime:end),u(2:ndime:end)]'
                   fprintf(fid_res,'End Values\n');
               elseif ndime==3
                   error('Problema bifasico no resuelto en 3D');
%                    fprintf(fid_res,'ComponentNames "X-DISPL" "Y-DISPL" "Z-DISPL"\n');
...
if protype==0 % MEJORARRRRRR!!!
   fprintf(fid_res,'Values\n');
   format = ['%d',repmat(' %.15g',1,ndn),'\n'];
   fprintf(fid_res,format,[in';udTotal]);
   fprintf(fid_res,'End Values\n');
end
...
for iSet = 1:nSet
   ...
   elseif protype==1 %AA
       ...
       %AA22
       phi_fluct = e_VarEst(iSet).phi_fluct;
       phi = e_VarEst(iSet).phi;
       porpr = e_VarEst(iSet).porpr;
       velflu = e_VarEst(iSet).velflu_total;
       mflu = e_VarEst(iSet).mflu;
       %AA22
   end  %AA
   ...
   GradPorMacroM= GradPorMacro{iSet};
   SGp = size(GradPorMacroM);  %AA
   if size(SGp,2)==3 %AA
       GradPorMM = reshape(GradPorMacroM,[SGp(1)*SGp(2),SGp(3)]);  %AA
   else
       GradPorMM = reshape(GradPorMacroM,SGp(1)*SGp(2),[]);  %AA
   end %AA
...
%          STRESS= reshape(STRESS,ntens*NPG_ELE,[]);
       ...
       case 1 %protype %AA
           switch struhyp %AA
               ...
               case {1,2} %Deformaci�n plana y Tensi�n plana
                   ...
                   tipoDatAlmac = 'Vector';
                   nomComponente = '"Vx" "Vy"';
                   fprintf(fid_res,['Result "Flux Velocity//On Gauss Points" "Load Analysis" %d %s ',...
                       'OnGaussPoints "',nomGaussSet,'"\n'],iStep,tipoDatAlmac);
                   fprintf(fid_res,'ComponentNames %s\n',nomComponente);
                   fprintf(fid_res,'Values\n');
                   format = ['%d',repmat([repmat(' %.15g',1,nvector),'\n'],1,NPG_ELE)];
                   m_Ind = reshape((1:npg*nvector)',nvector,[]);
                   fprintf(fid_res,format,[m_NumElem;velflu(m_Ind(:),:)]);
                   fprintf(fid_res,'End Values\n');
                   
                   % Contenido de masa del fluido (chi)         
                   tipoDatAlmac = 'Scalar';
                   nomComponente = '"Chi"';
                   fprintf(fid_res,['Result "Chi//On Gauss Points" "Load Analysis" %d %s ',...
                       'OnGaussPoints "',nomGaussSet,'"\n'],iStep,tipoDatAlmac);
                   fprintf(fid_res,'ComponentNames %s\n',nomComponente);
                   fprintf(fid_res,'Values\n');
                   format = ['%d',repmat([repmat(' %.15g',1,nscalar),'\n'],1,NPG_ELE)];
                   m_Ind = reshape((1:npg*nscalar)',nscalar,[]);
                   fprintf(fid_res,format,[m_NumElem;mflu(m_Ind(:),:)]);
                   fprintf(fid_res,'End Values\n');
                   %######################################################################################
               otherwise %struhyp
                   error(['GidPost: Deformaciones en los puntos de Gauss: Componentes: ',...
   ...
   %####################################################################################################
      ...
      elseif protype==1
          ...
          if e_VG.esME || isfield(e_VG,'omegaMicro') 
             s_tipoDato = 'Strain//Total//On Gauss Points';
             s_tipoDato_phi = 'Phi//Total//On Gauss Points';
             
          else
             s_tipoDato = 'Strain//On Gauss Points';
             s_tipoDato_phi = 'Phi//On Gauss Points';
          end
          f_ImprTensPG(fid_res,iStep,struhyp,ntens,NPG_ELE,nomGaussSet,s_tipoDato,m_NumElem,strain)
          f_ImprTensPG(fid_res,iStep,struhyp,nvector,NPG_ELE,nomGaussSet,s_tipoDato_phi,m_NumElem,phi)
          %El primer elemento logico es para analisis MULTIESCALA y el segundo elemento logico es para analisis RVE
          if e_VG.esME || isfield(e_VG,'omegaMicro') 
             %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
             % Fluctuacion de deformaciones sobre los puntos de Gauss
             s_tipoDato = 'Strain//Fluctuations//On Gauss Points';
             s_tipoDato_phi = 'Phi//Fluctuations//On Gauss Points';
             f_ImprTensPG(fid_res,iStep,struhyp,ntens,NPG_ELE,nomGaussSet,s_tipoDato,m_NumElem,eps_fluct)
             f_ImprTensPG(fid_res,iStep,struhyp,nvector,NPG_ELE,nomGaussSet,s_tipoDato_phi,m_NumElem,phi_fluct)
             %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
             % Deformacion macro total aplicada en cada PG
             s_tipoDato = 'Strain//Macro applied//Over Element';
             s_tipoDato_phi = 'Phi//Macro applied//Over Element';
             f_ImprTensPG(fid_res,iStep,struhyp,ntens,1,nomGaussUnicoSet,s_tipoDato,m_NumElem,DefMM) % AA: REVISAAAAAAAAAAAAAAAAAAAAR 
             %AA: DefMacro por DefMM
             f_ImprTensPG(fid_res,iStep,struhyp,nvector,1,nomGaussUnicoSet,s_tipoDato_phi,m_NumElem,GradPorMM) 
          end
      end
      %####################################################################################################
      %
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
   ...
   switch conshyp 
      case {1,14,15,16,17,52,100} %AA: add 14
         %
      case {2,9}
         % Plasticidad J2
      ...
      case {50,55,60,61}     
         ...
         elseif protype == 1
             f_PostProcME_Bif(iStep,m_NumElem,reshape(strain,ntens,npg,[]),...
                 reshape(phi,2,npg,[]),reshape(porpr,1,npg,[]),e_DatMatSet,hvar_new,e_VG)
         end % AA22
         %###################################################################################
...
if ntens>=4
    switch struhyp
       ...
       otherwise
          error(['GidPost: Deformaciones en los puntos de Gauss: Componentes: ',...
             'No definidas para esta hipotesis de carga']);
    end
    fprintf(fid_res,'Values\n');
    format = ['%d',repmat([repmat(' %.15g',1,ntens),'\n'],1,npg)];
    switch struhyp
...
%######################################################################################    
elseif ntens==2
    %Gradiente de poropresiones
    tipoDatAlmac = 'Vector';
    nomComponente = '"Phix" "Phiy"';
    fprintf(fid_res,['Result "',s_tipoDato,'" ',...
     '"Load Analysis" %d %s OnGaussPoints "',nomGaussSet,'"\n'],iStep,tipoDatAlmac);
    fprintf(fid_res,'ComponentNames %s\n',nomComponente);
